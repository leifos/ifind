    ...
    def search(self, query, offset):
        ...
        * ?  
        ...
        # There's a problem with the Whoosh query created from the users query
          ...
          raise SearchEngineError("Whoosh Query Engine", e, errorType="Whoosh", query=query)

        # Our Whoosh Index is empty
        except EmptyIndexError, e: 		
          raise SearchEngineError("Whoosh Query Engine", e, errorType="Whoosh")
		...
		# Check for a type error for resultsPerPage and when processing results
          ...
          if isinstance(self.resultsPerPage, int) == False:
            ...
            raise SearchEngineError("Whoosh Query Engine", e, note = note, resultsPerPageType = type(self.resultsPerPage))

          raise SearchEngineError("Whoosh Query Engine", e)
