...
class CaliforniaQuadraticYieldGenerator(YieldGenerator):
    ...
    def make_yields(self):
        ...
        a = -0.8 # The steepness of the curve
        foo = [0, 5, 10]
        k = random.choice(foo)
        for index in range(self.depth):
...
class SouthAfricaQuadraticYieldGenerator(YieldGenerator):
    ...
    def make_yields(self):
        ...
        k = random.choice(foo)
        for index in range(self.depth):
            quadratic_yield_array.append(SouthAfricaQuadraticYieldGenerator.sa_quadratic_function(index, k))

        return quadratic_yield_array

    @staticmethod
    def sa_quadratic_function(x, k):
